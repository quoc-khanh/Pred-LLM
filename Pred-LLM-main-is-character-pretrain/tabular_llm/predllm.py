...
from tabular_llm.predllm_dataset import PredLLMDataset, GReaTDataCollator, MyDataset
...
class PredLLM:
  ...
  ### TODO
    ...
    def pretrain(self, data_list, learning_rate=5e-5, resume_from_checkpoint: tp.Union[bool, str] = False):
        logging.info("Convert data into HuggingFace dataset object...")
        count = 0
        total = len(data_list)
        my_ds = MyDataset(tokenizer=self.tokenizer, numerical_modeling=numerical_modeling)
        for df in data_list:
            df_new = pd.concat([df, df], axis=0)
            df_new = df_new.reset_index(drop=True)
            my_ds.add_dataframe(df_new)
            count += 1
            print(f"Load {count}/{total} datasets")

        my_ds.set_tokenizer(self.tokenizer, numerical_modeling=numerical_modeling)
        ...
        great_trainer = GReaTTrainer(self.model, training_args, train_dataset=my_ds, tokenizer=self.tokenizer,
  ...
  ###
    ...
    def sample(self, n_samples: int, data: pd.DataFrame,
        ...
        with tqdm(total=n_samples) as pbar:
            ...
            while n_samples > already_generated:
                ...
                df_gen = _convert_text_to_tabular_data(text_data, pd.DataFrame(columns=self.columns), numerical_features=numerical_features)
    ...
    def sample_new(self, n_samples: int, data: pd.DataFrame, max_length: int = 100, task: str = "classification") -> pd.DataFrame:
        ...
        for feature in feature_names:
            df_gen = self.sample(n_samples=n_each_feature, max_length=max_length, data=data,
        ...
        y_train_gen = self.great_sample(prompts, max_length=max_length, data=data).iloc[:, -1:]
        ...
        while n_llm_pred < n_generative and n_tried < 5:
            ...
            for idx in invalid_indices:
                y_train_gen.iloc[idx, 0] = self.great_sample(prompts[idx], max_length=max_length, data=data).iloc[0, -1]
            n_tried += 1
        # find rows with flawed labels
        invalid_indices = np.where(pd.to_numeric(y_train_gen.iloc[:, 0], errors='coerce').notnull() == False)[0]
        # use previous labels
    ...
    def great_sample(self, starting_prompts: tp.Union[str, list[str]], data: pd.DataFrame, temperature: float = 0.7, max_length: int = 100,
        ...
        df_gen = _convert_text_to_tabular_data(decoded_data, pd.DataFrame(columns=self.columns), numerical_features=numerical_features)
