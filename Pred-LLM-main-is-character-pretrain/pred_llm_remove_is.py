...
# sys.path.append('/kaggle/working/Pred-LLM')

# # Change the working directory
# os.chdir("/kaggle/working/Pred-LLM")

print(os.getcwd())
print(os.listdir())

# Add the path to sys.path
sys.path.append('/content/Pred-LLM')

# Change the working directory
os.chdir('/content/Pred-LLM')
...
# Rename features of X_y_train_new to match X_y_train_df
                ...
                X_y_train_new = pd.concat([X_y_train_new, X_y_train_df[X_y_train_new.columns]], axis=0, ignore_index=True)
                X_train_new = X_y_train_new.iloc[:, :-1].to_numpy(dtype=float).reshape(-1, n_feature)
                y_train_new = X_y_train_new.iloc[:, -1:].to_numpy(dtype=float).reshape(-1, )
